...
var findUser = function(db, callback) {
    var cursor =db.collection('discuss').find( );
...
var findUserwithName = function(db,callback) {
    var cursor = db.collection('discuss').find({"fname":"Arunit"});
    cursor.each(function(err,doc) {
        assert.equal(err,null);
        if(doc != null)
        {
...
var findUserwithUniversity = function(db, callback) {
    var cursor = db.collection('discuss').find({"education.university":"UMKC"});
    cursor.each(function(err,doc){
       assert.equal(err,null);
       if(doc != null)
       {
